...
def person_page():
    ...
    if Config.is_python_backend():
        ...
        firstname_normalized = name_lower(firstname) if firstname else ""
        surname_normalized = name_lower(surname) if surname else ""
        
        # For now, proxy to OCaml for actual data retrieval (database access not migrated)
        # TODO: Implement Python database access
        bridge = OCamlBridge()
