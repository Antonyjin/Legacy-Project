...
class TestDateBoundaryConditions:
    ...
    def test_valid_month_range(self):
        ...
        for month in [1, 6, 12]:
            r = requests.get(f"{BASE_URL}?m=CAL&yg=2000&mg={month}&dg=15", timeout=5)
    ...
    def test_valid_day_range(self):
        ...
        # Test common days
        for day in [1, 15, 28]:
            r = requests.get(f"{BASE_URL}?m=CAL&yg=2000&mg=6&dg={day}", timeout=5)
    ...
    def test_calendar_handles_edge_dates(self):
        ...
        # Leap year Feb 29
        r = requests.get(f"{BASE_URL}?m=CAL&yg=2000&mg=2&dg=29", timeout=5)
        assert r.status_code == 200

        # Dec 31
        r = requests.get(f"{BASE_URL}?m=CAL&yg=2000&mg=12&dg=31", timeout=5)
        assert r.status_code == 200

    def test_calendar_accepts_year_under_2500(self):
        """OCaml Date.compress requires year < 2500"""
        # Year 2499 should work (< 2500)
        r = requests.get(f"{BASE_URL}?m=CAL&yg=2499", timeout=5)
        assert r.status_code == 200

        # Year 1800 should work
        r = requests.get(f"{BASE_URL}?m=CAL&yg=1800", timeout=5)
        assert r.status_code == 200
...
class TestDatePrecision:
    ...
    def test_person_page_displays_dates(self):
        ...
        r = requests.get(f"{BASE_URL}?p=Elizabeth&n=Windsor", timeout=5)
        assert r.status_code == 200
...
class TestCalendarTypes:
    ...
    def test_gregorian_calendar_default(self):
        ...
        r = requests.get(f"{BASE_URL}?m=CAL&yg=2000&mg=6&dg=15", timeout=5)
        assert r.status_code == 200
    ...
    def test_calendar_page_loads(self):
        ...
        r = requests.get(f"{BASE_URL}?m=CAL", timeout=5)
        assert r.status_code == 200
...
class TestDateEdgeCases:
    ...
    def test_zero_day_month_accepted(self):
        ...
        # Year-only date
        r = requests.get(f"{BASE_URL}?m=CAL&yg=1900", timeout=5)
        assert r.status_code == 200

    def test_historical_dates(self):
        """Test dates far in the past (historical genealogy)"""
        # Test medieval date (1200)
        r = requests.get(f"{BASE_URL}?m=CAL&yg=1200", timeout=5)
        assert r.status_code == 200

        # Test ancient date (100)
        r = requests.get(f"{BASE_URL}?m=CAL&yg=100", timeout=5)
        assert r.status_code == 200
    ...
    def test_very_large_year_handled(self):
        ...
        r = requests.get(f"{BASE_URL}?m=CAL&yg=3000", timeout=5)
    ...
    def test_invalid_month_handled(self):
        ...
        r = requests.get(f"{BASE_URL}?m=CAL&yg=2000&mg=13", timeout=5)
        # Should either work or return client error, not server error
        assert r.status_code < 500

        # Month 0 (year-only date)
        r = requests.get(f"{BASE_URL}?m=CAL&yg=2000&mg=0", timeout=5)
        assert r.status_code < 500

    def test_invalid_day_handled(self):
        """Test that invalid day values are handled gracefully"""
        # Day 32 (invalid)
        r = requests.get(f"{BASE_URL}?m=CAL&yg=2000&mg=6&dg=32", timeout=5)
        ...
        r = requests.get(f"{BASE_URL}?m=CAL&yg=2000&mg=6&dg=0", timeout=5)
        assert r.status_code < 500
...
class TestDateDisplay:
    ...
    def test_death_date_display(self):
        ...
        r = requests.get(f"{BASE_URL}?p=Philip&n=Mountbatten", timeout=5)
        assert r.status_code == 200
        # Philip died in 2021
    ...
    def test_marriage_date_display(self):
        ...
        r = requests.get(f"{BASE_URL}?m=F&p=Charles&n=Windsor", timeout=5)
        assert r.status_code == 200
...
class TestDateCompression:
    ...
    def test_year_boundary_2500(self):
        ...
        r = requests.get(f"{BASE_URL}?m=CAL&yg=2499&mg=6&dg=15", timeout=5)
        assert r.status_code == 200

        # Year 2500 exceeds compression limit but should still work
        # (stored as uncompressed Cdate instead of Cgregorian)
        r = requests.get(f"{BASE_URL}?m=CAL&yg=2500&mg=6&dg=15", timeout=5)
