...
let mutil_extract_param _ =
  let test str sep eq exp =
    (check (list (pair string string))) "" exp (Mutil.extract_param str sep eq)
  in
  test "key1=value1&key2=value2" '&' '=' [("key1", "value1"); ("key2", "value2")];
  test "m=RL&n=I1&o=I2" '&' '=' [("m", "RL"); ("n", "I1"); ("o", "I2")];
  test "key=value" '&' '=' [("key", "value")];
  test "" '&' '=' [];
  test "key1=&key2=value2" '&' '=' [("key1", ""); ("key2", "value2")];
  ...
  [
    ( "mutil",
      [
        ...
        test_case "Mutil.string_of_int_sep" `Quick mutil_string_of_int_sep;
        test_case "Mutil.extract_param" `Quick mutil_extract_param;
      ] );
